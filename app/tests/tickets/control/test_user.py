...
class UserSettings2FATest(SoupTest):
    ...
    def test_confirm_totp(self):
        self.client.post(
            '/control/settings/2fa/add',
            {'devicetype': 'totp', 'name': 'Foo'},
            follow=True,
        )
        d = TOTPDevice.objects.first()
        ...
        totp.time = time.time()
        r = self.client.post(
            '/control/settings/2fa/totp/{}/confirm'.format(d.pk),
            {'token': str(totp.token()), 'activate': 'on'},
            follow=True,
        )
        d.refresh_from_db()
        assert d.confirmed
...
class UserSettingsNotificationsTest(SoupTest):
    ...
    def test_event_enabled_disable(self):
        ...
        self.client.post(
            '/control/settings/notifications/?event={}'.format(self.event.pk),
            {'mail:pretix.event.order.placed': 'off'},
        )
        assert (
            self.user.notification_settings.get(
