...
class CheckoutTestCase(BaseCheckoutTestCase, TestCase):
    ...
    def test_question_file_upload(self):
        ...
        response = self.client.post(
            '/%s/%s/checkout/questions/' % (self.orga.slug, self.event.slug),
            {'%s-question_%s' % (cr1.id, q1.id): f, 'email': 'admin@localhost'},
            follow=True,
        )
        self.assertRedirects(
            response,
    ...
    def test_attendee_email_required(self):
        ...
        response = self.client.post(
            '/%s/%s/checkout/questions/' % (self.orga.slug, self.event.slug),
            {'%s-attendee_email' % cr1.id: '', 'email': 'admin@localhost'},
            follow=True,
        )
        doc = BeautifulSoup(response.content.decode(), 'lxml')
        self.assertGreaterEqual(len(doc.select('.has-error')), 1)
        ...
        response = self.client.post(
            '/%s/%s/checkout/questions/' % (self.orga.slug, self.event.slug),
            {'%s-attendee_email' % cr1.id: 'foo@localhost', 'email': 'admin@localhost'},
            follow=True,
        )
        self.assertRedirects(
            response,
    ...
    def test_attendee_company_required(self):
        ...
        response = self.client.post(
            '/%s/%s/checkout/questions/' % (self.orga.slug, self.event.slug),
            {'%s-company' % cr1.id: '', 'email': 'admin@localhost'},
            follow=True,
        )
        doc = BeautifulSoup(response.content.decode(), 'lxml')
        self.assertGreaterEqual(len(doc.select('.has-error')), 1)
        ...
        response = self.client.post(
            '/%s/%s/checkout/questions/' % (self.orga.slug, self.event.slug),
            {'%s-company' % cr1.id: 'foobar', 'email': 'admin@localhost'},
            follow=True,
        )
        self.assertRedirects(
            response,
    ...
    def test_attendee_name_required(self):
        ...
        response = self.client.post(
            '/%s/%s/checkout/questions/' % (self.orga.slug, self.event.slug),
            {'%s-attendee_name_parts_0' % cr1.id: '', 'email': 'admin@localhost'},
            follow=True,
        )
        doc = BeautifulSoup(response.content.decode(), 'lxml')
        self.assertGreaterEqual(len(doc.select('.has-error')), 1)
        ...
        response = self.client.post(
            '/%s/%s/checkout/questions/' % (self.orga.slug, self.event.slug),
            {'%s-attendee_name_parts_0' % cr1.id: 'Peter', 'email': 'admin@localhost'},
            follow=True,
        )
        self.assertRedirects(
            response,
    ...
    def test_invoice_address_hidden_for_free(self):
        ...
        self.assertRedirects(
            response,
            '/%s/%s/checkout/confirm/' % (self.orga.slug, self.event.slug),
            target_status_code=200,
        )
    ...
    def test_invoice_name_required(self):
        ...
        response = self.client.post(
            '/%s/%s/checkout/questions/' % (self.orga.slug, self.event.slug),
            {'name_parts_0': 'Raphael', 'email': 'admin@localhost'},
            follow=True,
        )
        self.assertRedirects(
            response,
    ...
    def test_payment_country_allowed(self):
        ...
        ia = InvoiceAddress.objects.create(
            ...
            vat_id_validated=True,
            country=Country('DE'),
            name_parts={'full_name': 'Foo', '_scheme': 'full'},
            name_cached='Foo',
            street='Foo',
        )
        self._set_session('invoice_address', ia.pk)
    ...
    def test_giftcard_full_with_multiple(self):
        ...
        response = self.client.post(
            '/%s/%s/checkout/payment/' % (self.orga.slug, self.event.slug),
            {'payment': 'giftcard', 'giftcard': gc2.secret},
            follow=True,
        )
        self.assertRedirects(
            response,
    ...
    def test_voucher_double(self):
        ...
        with scopes_disabled():
            self.assertEqual(
                CartPosition.objects.filter(cart_id=self.session_key, voucher=v).count(),
                1,
            )
            self.assertEqual(len(doc.select('.alert-danger')), 1)
            self.assertFalse(Order.objects.exists())
    ...
    def test_addons_as_first_step(self):
        ...
        self.assertRedirects(
            response,
            '/%s/%s/checkout/addons/' % (self.orga.slug, self.event.slug),
            target_status_code=200,
        )
    ...
    def test_confirm_subevent_presale_over(self):
        with scopes_disabled():
            ...
            se = self.event.subevents.create(
                name='Foo',
                date_from=now(),
                presale_end=now() - datetime.timedelta(days=1),
            )
            CartPosition.objects.create(
                event=self.event,
                cart_id=self.session_key,
...
class QuestionsTestCase(BaseCheckoutTestCase, TestCase):
    def test_timezone(self):
        ...
        with scopes_disabled():
            ...
            q3 = Question.objects.create(
                event=self.event,
                question='When are you going to arrive?',
                type=Question.TYPE_DATETIME,
                required=True,
            )
            self.ticket.questions.add(q1)
    ...
    def test_addon_questions(self):
        ...
        with scopes_disabled():
            self.assertEqual(
                OrderPosition.objects.filter(item=self.ticket).first().answers.first().answer,
                '12',
            )
            self.assertEqual(
                OrderPosition.objects.filter(item=self.workshop1).first().answers.first().answer,
                '12',
            )
    ...
    def test_questions(self):
        with scopes_disabled():
            ...
            q2 = Question.objects.create(
                event=self.event,
                question='How have you heard from us?',
    ...
    def _test_question_input(self, data, should_fail):
        ...
        pl = {('%s-question_%s' % (cr1.id, k.id)): v for k, v in data.items() if v != 'False'}
        ...
        with scopes_disabled():
            for k, v in data.items():
                ...
                if k.type in Question.OPTION_TYPES:
    ...
    @scopes_disabled()
    def _setup_dependency_questions(self, root_question_type=Question.TYPE_CHOICE):
        self.q1 = self.event.questions.create(
            event=self.event,
            question='What industry are you in?',
            type=root_question_type,
            required=True,
        )
        ...
        self.q2a = self.event.questions.create(
            ...
            required=False,
            dependency_question=self.q1,
            dependency_values=['TECH', 'IT'],
        )
        ...
        self.q3 = self.event.questions.create(
            ...
            required=False,
            dependency_question=self.q2a,
            dependency_values=['DEV'],
        )
    ...
    def test_question_dependencies_first_path(self):
        ...
        self._test_question_input({self.q1: 'HEALTH', self.q2b: 'NURSE'}, should_fail=False)
    ...
    def test_question_dependencies_with_dropdown_parent(self):
        self._setup_dependency_questions(root_question_type=Question.TYPE_CHOICE_DROPDOWN)
        self._test_question_input(
            {
                self.q1: 'TECH',
                self.q2a: 'DEV',
...
class CheckoutBundleTest(BaseCheckoutTestCase, TestCase):
    ...
    def test_bundle_with_count(self):
        ...
        oid = _perform_order(
            self.event,
            'manual',
            [self.cp1.pk, self.bundled1.pk, bundled2.pk],
            'admin@example.org',
            'en',
            None,
            {},
    ...
    def test_expired_bundled_and_addon(self):
        ...
        oid = _perform_order(
            self.event,
            'manual',
            [self.cp1.pk, self.bundled1.pk, a.pk],
            'admin@example.org',
            'en',
            None,
            {},
    ...
    def test_addon_and_bundle_through_frontend_stack(self):
        ...
        self.trans.save()
        ItemAddOn.objects.create(base_item=self.ticket, addon_category=cat, min_count=1, price_included=True)
        CartPosition.objects.create(
            event=self.event,
            cart_id=self.session_key,
            item=self.trans,
...
class CheckoutSeatingTest(BaseCheckoutTestCase, TestCase):
    ...
    def setUp(self):
        super().setUp()
        self.plan = SeatingPlan.objects.create(name='Plan', organizer=self.orga, layout='{}')
    ...
    def test_seat_taken(self):
        CartPosition.objects.create(
            event=self.event,
            cart_id=self.session_key + '_other',
            item=self.ticket,
            price=21.5,
            expires=now() + timedelta(minutes=10),
            seat=self.seat_a1,
