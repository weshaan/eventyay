...
class ReviewDashboard(EventPermissionRequired, BaseSubmissionList):
    ...
    def get_queryset(self):
        ...
        queryset = (
            ...
            .annotate(
                ...
                review_nonnull_count=Count('reviews', distinct=True, filter=Q(reviews__score__isnull=False)),
            )
        )
        queryset = self.filter_range(queryset)

        user_reviews = Review.objects.filter(user=self.request.user, submission_id=OuterRef('pk')).values('score')
    ...
    def submissions_reviewed(self):
        return Review.objects.filter(user=self.request.user, submission__event=self.request.event).values_list(
    ...
    def post(self, request, *args, **kwargs):
        ...
        for key, value in request.POST.items():
            ...
            code = key.strip('s-')
            try:
                submission = request.event.submissions.filter(state=SubmissionStates.SUBMITTED).get(code=code)
            ...
            if not request.user.has_perm('base.' + value + '_submission', submission):
                ...
                continue
            if pending:
                submission.pending_state = SubmissionStates.ACCEPTED if value == 'accept' else SubmissionStates.REJECTED
                submission.save()
            else:
                getattr(submission, value)(person=request.user)
            total[value] += 1
        if total['accept'] or total['reject']:
            msg = str(_('Success! {accepted} proposals were accepted, {rejected} proposals were rejected.')).format(
                accepted=total['accept'], rejected=total['reject']
            )
            if total['error']:
                msg += ' ' + str(_('We were unable to change the state of {count} proposals.')).format(
                    count=total['error']
                )
            ...
            messages.error(
                request,
                str(_('We were unable to change the state of all {count} proposals.')).format(count=total['error']),
            )
...
class BulkReview(EventPermissionRequired, TemplateView):
    ...
    def submissions(self):
        submissions = get_reviewable_submissions(event=self.request.event, user=self.request.user).prefetch_related(
            'speakers'
        )
    ...
    @cached_property
    def categories(self):
        return self.request.event.score_categories.all().filter(active=True).prefetch_related('limit_tracks', 'scores')
    ...
    def forms(self):
        own_reviews = {
            ...
            for review in self.request.event.reviews.filter(user=self.request.user, submission__in=self.submissions)
        ...
        return {
            submission.code: ReviewForm(
                ...
                categories=(categories[submission.track_id] if submission.track_id else []) + categories[None],
        ...
        return [
            {
                ...
                'score_fields': [self.forms[submission.code].get_score_field(category) for category in self.categories],
            }
            for submission in self.submissions
        ]
...
class ReviewSubmission(ReviewViewMixin, PermissionRequired, CreateOrUpdateView):
    ...
    def review_display(self):
        if self.object:
            ...
            return {
                ...
                'text': review.text,
                'user': review.user,
                'answers': [review.answers.filter(question=question).first() for question in self.qform.queryset],
            }
    ...
    def has_anonymised_review(self):
        return (
            self.request.event.review_phases.filter(can_see_speaker_names=False).exists()
    ...
    def get_scores_for_review(self, review):
        scores = []
        score_format = self.request.event.review_settings.get('score_format', 'words_numbers')
    ...
    def get_context_data(self, **kwargs):
        ...
        result['total_reviews'] = get_missing_reviews(self.request.event, self.request.user).count() + result['done']
    ...
    def get_success_url(self) -> str:
        ...
        if not next_submission:
            ignored_submissions = [self.submission.pk] if action == 'skip_for_now' else []
            next_submission = get_missing_reviews(
                self.request.event,
                self.request.user,
                ignore=ignored_submissions,
...
class RegenerateDecisionMails(EventPermissionRequired, ActionConfirmMixin, TemplateView):
    ...
    def post(self, request, **kwargs):
        ...
        messages.success(
            request,
            _('{count} emails were generated and placed in the outbox.').format(count=self.count),
        )
