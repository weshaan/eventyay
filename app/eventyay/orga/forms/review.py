...
class ReviewForm(ReadOnlyFlag, forms.ModelForm):
    def __init__(
        ...
        **kwargs,
        ...
        self.scores = (
            {score.category: score.id for score in self.instance.scores.all().select_related('category')}
            if self.instance.id
            else {}
        )
    ...
    def build_score_field(self, category, read_only=False, initial=None, hide_optional=False):
        ...
        for score in category.scores.all():
            choices.append(
                (
                    score.id,
                    score.format(self.event.review_settings.get('score_format', 'words_numbers')),
                )
            )
    ...
    def clean(self):
        ...
        if not cleaned_data.get('text') and self.event.review_settings['text_mandatory'] and not self.allow_empty:
        ...
        for category in self.categories:
            ...
            if score in ('', None) and category.required and not self.allow_empty:
                self.add_error(f'score_{category.id}', _('Please provide a review score!'))
        return cleaned_data
...
class ReviewerForProposalForm(ReviewAssignmentForm):
    def __init__(self, *args, **kwargs):
        ...
        for submission in self.submissions:
            self.fields[submission.code] = forms.MultipleChoiceField(
                ...
                initial=list(submission.assigned_reviewers.values_list('id', flat=True)),
    ...
    def get_review_choices_by_track(self, track):
        ...
        result = [
            (reviewer.id, reviewer.fullname) for reviewer in sorted(reviewers, key=lambda x: (x.fullname or '').lower())
        ]
...
class ProposalForReviewerForm(ReviewAssignmentForm):
    def __init__(self, *args, **kwargs):
        ...
        for submission in self.submissions:
            self.submissions_by_track[submission.track_id].add((submission.id, submission.title))
    ...
    def get_submission_choices_by_tracks(self, track_limit):
        ...
        return sorted(list(submissions), key=lambda submission: (submission[1] or '').lower())
...
class ReviewAssignImportForm(DirectionForm):
    ...
    def _get_submission(self, text):
        if not self._submissions_cache:
            self._submissions_cache = {sub.code: sub for sub in self.event.submissions.all()}
        try:
