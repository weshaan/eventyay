...
def _strip_public_event_security(result):
    # We never want to show auth information on public event list/detail endpoints, as the
    # display with redoc really sucks. We disabled this globally, but it snuck back in.
    public_event_paths = (
        '/api/v1/events/',
        '/api/v1/events/{event}/',
        '/api/events/',
    ...
    for path in public_event_paths:
        operation = result.get('paths', {}).get(path, {}).get('get')
...
def postprocess_schema(result, generator, request, public):
    ...
    _strip_public_event_security(result)
